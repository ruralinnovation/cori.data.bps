...
Access and analyze U.S. Census Bureau Building Permits Survey (BPS) data covering new residential units authorized from 2000 to present.
...
**Census API Key** (required): Get one at http://api.census.gov/data/key_signup.html

```r
Sys.setenv(CENSUS_API_KEY = "your_api_key_here")
```

**S3 access** (for `read_bds_from_s3()` and upload): Install `cori.data.s3`. Reads fall back to CORI's credential-vending endpoint if no local AWS credentials are configured.
...
All data is returned in tidy (long) format with one row per `geoid x year x variable` combination.
...
The `agg_var` column contains `population / 1,000`, suitable for population-weighted averaging of rates.
...
**Structure types:** Total includes single-family, 2-family, 3-to-4-family, and 5-plus-family units.
